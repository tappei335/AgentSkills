...
GPT-6 now includes a user-requested official-guidance pilot. Its [adoption record](variants/gpt-6/_shared/evidence.md) maps the source to this repository, records the exception to measured-only adoption, and defines behavioral evaluation cases. Model-quality improvements remain unmeasured. GPT-5.6 retains its previous instructions. The source is [OpenAI's GPT-6 guide](https://developers.openai.com/api/docs/guides/latest-model).

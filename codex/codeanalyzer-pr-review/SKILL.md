...
description: Review a CodeAnalyzer GitHub PR identified by number, URL, branch, or commit, and publish through its review bot by default. Requires a fetched PR; no local-diff substitute. Honor requests for chat-only review.
---
...
Read [comment-format.md](references/comment-format.md) when composing the review. It defines the required summary, findings, optional checks, and examples for initial reviews and re-reviews.

...
Do not stop at a chat-only summary unless the user explicitly says `dry-run`, `chat only`, `do not comment`, `コメントしないで`, or equivalent.

## Inputs

Accept a PR number, PR URL, branch name, commit SHA, or commit range. If the PR cannot be identified, ask one concise question for the missing PR identifier.
...
2. Read repository instructions that apply to the touched paths, such as `CLAUDE.md`, `AGENTS.md`, `.github/pull_request_template.md`, `CONTRIBUTING.md`, path-local rules, and relevant CodeAnalyzer design or roadmap docs.
...
4. Fetch PR metadata, changed files, diff or patches, existing comments, review threads, and CI/check status.
5. Review for correctness, regressions, architecture invariant violations, missing tests or fixtures, documentation drift, and CI/check risk.
6. Classify every concern as either PR-scoped, out-of-scope issue material, or not worth mentioning. Do not leave vague "follow-up later" items in the review.
7. Prefer concrete findings over general suggestions. Each finding should include the file path, line or diff location when available, severity, type, category, evidence, impact, and the minimal fix direction.
8. Run the final skeptical pass before publishing.
9. Publish the review through the `CodeAnalyzer Review Bot` GitHub Actions workflow unless the user explicitly requested no posting.
10. Report in chat what was posted and any CI/check gaps that matter.

Use GitHub connector tools, `gh pr view`, `gh pr diff`, and `gh pr checks` for read-only inspection. Do not publish reviews with the user's GitHub identity unless the user explicitly asks for direct local posting.

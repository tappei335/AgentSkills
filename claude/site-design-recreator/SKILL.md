...
- The **main agent/manager** critiques screenshots, diffs, DOM/CSS reconstruction, responsive behavior, and image-use policy. The manager must not implement a pass that should be delegated. The manager runs Playwright captures and comparisons itself via `Bash`.
- Spawn a **designer/worker** sub-agent to implement fixes from the manager's review list: `Agent(subagent_type="general-purpose", name="designer", run_in_background=false)`. The loop is sequential, so run the designer synchronously; the manager cannot re-capture until the pass lands. The designer must edit files in its assigned write scope and must not approve its own work.
- Continue the **same** designer across fix passes with `SendMessage(to="designer")` so it keeps the reconstruction plan, prior fix lists, and file knowledge in context. Spawn a fresh designer only when the previous one is stale, mis-scoped, or repeatedly fails a fix item.
